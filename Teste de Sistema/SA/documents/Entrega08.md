...
## 8.4 Preparação e execução

Configure `backend/api/.env` conforme `.env.example` e mantenha o PostgreSQL ativo.
...
O comando do Playwright inicia o backend, sincroniza o banco, aplica a carga inicial e inicia o frontend automaticamente.

## 8.5 Critério de aprovação

A entrega é aprovada quando todas as suítes terminam sem falhas, o build do frontend é concluído e os fluxos CT-01 a CT-13 apresentam os resultados esperados. Em caso de falha, o relatório HTML e os traces do Playwright devem ser anexados como evidência.

## 8.6 Resultado da execução

Execução realizada em **22/06/2026**:

| Suíte | Resultado |
|---|---|
| API unitária e integrada ao PostgreSQL | 17 testes aprovados, 0 reprovados |
| Sistema com Playwright/Chromium | 6 testes aprovados, 0 reprovados |
